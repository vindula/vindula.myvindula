...
##parameters=images=0, folders=0, subimages=0, others=0, vindulaNew=1, cliente=1
...
if vindulaNew:
    # We don't need the full objects for the folders
    if is_topic:
        result['vindulaNew'] = context.queryCatalog(portal_type='VindulaNews')
    else:
        result['vindulaNew'] = context.getFolderContents({'portal_type': ('VindulaNews',)})
        
if cliente:
    # We don't need the full objects for the folders
    try:
        if is_topic:
            result['cliente'] = context.queryCatalog(portal_type='Cliente')
        else:
            result['cliente'] = context.getFolderContents({'portal_type': ('Cliente',)})
    except:
        result['cliente'] = None
...
if others:
    ...
    filtered = [p_type for p_type in searchContentTypes
                if p_type not in ('Image', 'Folder','Cliente', 'VindulaNews')]
    if filtered:
